  ...
  return (
    <Popover open={panelOpen} onOpenChange={setPanelOpen}>
      <PopoverTrigger asChild>
        <button
          type="button"
          aria-label={connected ? t("wallet.walletLabel", { address: truncatedAddress ?? "" }) : t("wallet.connect")}
          ...
          <div className="flex items-center gap-2 pr-8">
            ...
            <div className="min-w-0">
              <p className="text-xs font-semibold uppercase tracking-widest text-forest/60">
                {walletName ? t("wallet.namedWallet", { name: walletName }) : t("wallet.solanaWallet")}
              </p>
              {connected && truncatedAddress && address ? (
                <a
                  ...
                  title={t("wallet.viewOnJupiter")}
                >
                  {truncatedAddress}
                </a>
              ) : (
                <p className="text-sm text-forest/70">{t("wallet.notConnected")}</p>
              )}
            </div>
          </div>
          ...
          {connected ? (
            <div className="rounded-xl border border-forest/10 bg-white/70 px-3 py-2.5 text-sm">
              <div className="flex items-center justify-between gap-2">
                <span className="text-forest/60 gaine-word gaine-word-sm">{t("wallet.balance")}</span>
                <span className="font-mono font-medium text-forest">
                  {balanceLoading ? "…" : (gaineBalance ?? 0).toLocaleString()}
                </span>
              </div>
              {hasGaine ? (
                <p className="mt-1 text-xs text-gold-deep">{t("communityChat.holderUnlocked")}</p>
              ) : (
                <p className="mt-1 text-xs text-forest/55">{t("communityChat.holdToEnter")}</p>
              )}
            </div>
          ) : null}
          ...
          <div className="flex flex-col gap-2">
            {connected ? (
              ...
              <Button
                ...
                disabled={connecting}
              >
                {connecting ? t("wallet.connecting") : t("wallet.connectCta")}
              </Button>
            )}
          </div>
        </div>
